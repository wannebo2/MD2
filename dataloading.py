...
class TrajectoryDataset(Dataset):
    def __init__(self,directory):
        ...
        self.NoiseScales = [1,1,1,0,0.1,0.1,0.1,1,1,1]
    ...
    def __getitem__(self,number): #loads some random frames from trajectory number number
        ...
        mnTime = min(steps)
        atmEmbeds,locs = input_utils.loadDCD(f,steps,pdb,psf,timeConstant = 48.88821,tolerance = 0.0001)
        LocLen = len(locs[0])
        noised = []
        ...
        return [atmEmbeds,noised],locs #not sure if this is actually the right format for this, will probably have to come back and fix
